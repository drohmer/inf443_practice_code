...
float ShadowCalculation(vec4 fragPosLightSpace, vec3 L, vec3 N)
{
    ...
    float bias = max(0.002 * (1.0 - dot(N, L)), 0.005);//0.005;
    float shadow = 0.0;
    vec2 texelSize = 1.0 / textureSize(depth_map_texture, 0);
    for(int x = -2; x <= 2; ++x)
    {
        for(int y = -2; y <= 2; ++y)
        {
...
void main()
{
	...
	vec3 color_shading = (Ka + Kd * (1.0-shadow) * diffuse_component) * color_object + Ks * (1.0-shadow) * specular_component * vec3(1.0, 1.0, 1.0);
